...
describe('RepliesDeleteUseCase', () => {
    it('should orchestrating the delete replies action correctly', async () => {
        ...
        mockRepliesRepository.checkRepliesIsExist = jest.fn().mockImplementation(() => Promise.resolve())
        mockRepliesRepository.checkRepliesOwner = jest.fn().mockImplementation(() => Promise.resolve())
        mockRepliesRepository.deleteReplies = jest.fn().mockImplementation(() => Promise.resolve())
        ...
        expect(mockRepliesRepository.checkRepliesIsExist).toHaveBeenCalledWith(useCasePayload.replyId)
        expect(mockRepliesRepository.checkRepliesOwner).toHaveBeenCalledWith(useCasePayload.replyId, useCasePayload.owner)
        expect(mockRepliesRepository.deleteReplies).toHaveBeenCalledWith(useCasePayload.replyId)
    })
})